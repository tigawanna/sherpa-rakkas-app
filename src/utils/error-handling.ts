...
export function extractFieldFromPrismaErrorMessage(message: string): string | null {
    const regex = /\(`([^`]+)`\)/;
...
if(error?.meta?.target && error?.meta?.target?.includes(field)){
    return `${prismaErrorMessages[error.code as keyof typeof prismaErrorMessages]}`;
}
}
